...
#t <- p_partielle %>% group_by(id_pe, year,type) %>% summarise(n = n()) %>% na.omit() %>% filter(n > 1)
#t <- p_totale %>% group_by(id_pe, year,type) %>% summarise(n = n()) %>% na.omit() %>% filter(n > 1)

# virer les placette ou il y a de la plantation
p_totale <- p_totale %>% rename(perturb = origine)
pert <- bind_rows(p_partielle, p_totale)
plant_placette <- pert %>% filter(type %in% c("plantation", "partial_plantation"))
plant_placette <- plant_placette$id_pe

# supprimer les placettes avec plantation de sap_data
sap_data <- sap_data %>% filter(!id_pe %in% plant_placette)

# recuperer la derniere perturbation pour chaque id_pe_mes cad la plus recente (year max)
pert1 <- pert %>% group_by(id_pe, no_mes, type) %>% summarise(year = max(year))

# merge sap and pert1
sap_pert <- merge(sap_data, pert1, all.x = TRUE)

# colonne deltaT = annee de la mesure - annee de la perturbation et 0 si pas de perturbation
sap_pert <- sap_pert %>%
    mutate(deltaT = ifelse(is.na(year), 0, as.numeric(year_measured) - as.numeric(year)))

pep_xy <- pep_xy %>% filter(!id_pe %in% plant_placette)