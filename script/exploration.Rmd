...
pep_ori <- st_read("data/PEP_GPKG/PEP.gpkg",
    layer = "pee_ori_sond")
...
```{r}
recrutement_plus <- merge(data.frame(recrutement), data.frame(pep_mes_xy))
```

carte des recrutements pour la premiière version d'inventaire : "1er inv. 1975 à 1981"
pour acer rubrum
...
recrutement %>%
    filter(essence == "ERR") %>%
    mapview(zcol = "no_mes", cex = "recrutement")
```
...
Ajout des infos de placette aux infos d'arbre
    
```{r}
recrutement <- recrutement %>%
    st_join(pep_mes_xy, by = c("id_pe", "no_mes"))
```
...
```{r}
ggplot(recrutement) +
    geom_point(aes(x = date_sond, y = recrutement, color = essence))
```
...
```{r}
important_essence <- c("BOP","EPB","EPN","ERR","ERS","PET","PIG","SAB","NA")
...
Est ce que les plots sont bien répartis dans l'espace chaque année ?
Pas du tout, les placettes sont très concentrées chaque année dans un petit espace.
