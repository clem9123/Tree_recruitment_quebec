...
- [ ] Etudier les différentes perturbation
   - [x] Général overview
   - [ ] Séparer les perturbations par type
   - [ ] Séparer les perturbation par importance
...
```{r, new_data}
sp <- perturbation[which(!is.na(perturbation$perturb) &
    perturbation$an_perturb %in% 1971:2023),]
ssp <- sp %>% select(id_pe, perturb, an_perturb) %>% unique()
...
gaule_perturb <- t  %>%
    ...
    mutate(is_perturb = ifelse(is.na(deltaT),0,1)) %>%
    mutate(is_gaule = ifelse(all_cl == 0,0,1))
gaule_perturb_imp <- gaule_perturb %>%
    filter(essence %in% c("SAB","EPN","BOP","ERR","ERS","BOJ"))
gaule_perturb_imp <- gaule_perturb_imp %>%
    mutate(perturb = ifelse(is.na(perturb),
        "None", perturb))
```
...
ggplot(gaule_perturb_imp) +
    geom_bar(aes(x = factor(is_gaule), fill = factor(is_perturb))) +
    facet_wrap(~essence)
```
